...
$res=$conn->query("SELECT c.id, m.name, m.price, c.qty, m.id as mid FROM cart c JOIN menu m ON m.id=c.item_id WHERE user_id=$uid");
$total=0; $items=[];
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $oid=$conn->insert_id;
  foreach($items as $i){
    $conn->query("INSERT INTO order_items (order_id,item_id,qty,price) VALUES ($oid,{$i['mid']},{$i['qty']},{$i['price']})");
  }
